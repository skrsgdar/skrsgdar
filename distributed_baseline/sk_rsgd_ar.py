...
import numpy as np
import torch
from utils import discrete_tensor_extra, clip_grad_by_l2norm
...
if __name__ == '__main__':
    ...
    parser.add_argument("--scale_noise", help="scale of gaussian noise", type=float, required=True)
    ...
    parser.add_argument("--extra_in_l2_squared", type=float, required=True)
    ...
    for i in range(1, args.n_round + 1):
        ...
        for client_id in range(args.n_client):
            ...
            logging.info(f"Round #{i}, Client #{client_id}: Training loss {loss_avg} with gaussian scale {args.scale_noise}")

            with torch.no_grad():
                # calculate and scale delta
                scaled_deltas = {_: args.scale_parameter * (client_params[_] - server_params[_]) for _ in
                                 server_params.keys()}
                # discrete
                for key, delta in scaled_deltas.items():
                    scaled_deltas[key].data = discrete_tensor_extra(delta.data, args.extra_in_l2_squared)

                # inject sk noise into delta
                for key, delta in scaled_deltas.items():
                    scaled_deltas[key].data = delta.data + skellam_noise(delta.size(), args.possion_rate, device)
        ...
        with torch.no_grad():
            ...
            for key, delta in avg_deltas.items():
                avg_deltas[key] = delta / args.scale_parameter
