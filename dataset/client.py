...
def read_file(path):
    ...
    for sample in data:
        for i in range(9, 23):
            sample[i] = (sample[i] - mins[i]) / (maxs[i] - mins[i])
...
def getClientDataset_by_batchsz(n_client, batch_size, num_workers, drop_last=False, root="../raw_data/CLIENT/"):
    ...
    # wrap into dataset
    train_dataset = [ClientDataset(_, raw_data) for _ in train_partition]
    test_dataset = [ClientDataset(_, raw_data) for _ in test_partition]
...
def getClientDataset_by_sampling_rate(n_client, q, num_workers, drop_last=False, root="../raw_data/CLIENT/"):
    raw_data = read_file(os.path.join(root, __RAWDATA__))
...
class ClientDataset(object):
    ...
    def __getitem__(self, index):
        ...
        for i, value in enumerate(sample):
            ...
            if i == 0:
                # gender
                embedding = [np.float32(value == 1), np.float32(value == 2)]
                assert sum(embedding) == 1
                data += embedding
            elif i == 1:
                # education
                embedding = [np.float32(value == _) for _ in range(0, 7)]
                assert sum(embedding) == 1
                data += embedding
            elif i == 2:
                # marital status
                embedding = [np.float32(value == _) for _ in range(0, 4)]
                assert sum(embedding) == 1
                data += embedding
            elif 3 <= i <= 8:
                # marital status
                embedding = [np.float32(value == _) for _ in range(-2, 9)]
                assert sum(embedding) == 1, f"Wrong value {value} {i}"
                data += embedding
            elif i >= 9:
                assert 0 <= value <= 1
                data += [value]
...
if __name__ == '__main__':
    train_loaders, test_loaders = getClientDataset_by_batchsz(10, 32, 3)
